 ...
 *
 * Mirrors the neck-posture module's guard: the on-device MediaPipe runtime
 * will, by default, POST anonymous usage counters to Google's ML telemetry
 * endpoint. No webcam data is involved, but Posturama promises that camera
 * processing stays local, so we block those calls outright.
 *
